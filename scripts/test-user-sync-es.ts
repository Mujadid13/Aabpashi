...
// Override MONGO_URI for local testing (use localhost instead of mongodb container name)
const MONGO_URI = process.env.MONGO_URI || 'mongodb://localhost:27017/aabpashi';
...
const AABPASHI_API_KEY = "aabpashi_3e56d3caf71680e66977dcda9da3c05b19f2f65567297e9260851b62acb675fb";

// Test configuration
const config = {
  aabpashiApiUrl: "http://aabpashi-app:3000/api/sync/create-user",
...
async function testSyncAPI() {
  ...
  try {
    ...
    const response = await fetch(`http://aabpashi-app:3000/api/sync/create-user`, {
      method: 'POST',
      headers: {
...
//     const fieldMapper = syncFieldMapper;
//     
...
async function verifyUserInFarmovation(userId: string) {
  if (!userId) {
